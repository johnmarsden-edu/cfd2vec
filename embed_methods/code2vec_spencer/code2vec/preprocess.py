...
def process_file(file_path, data_file_role, dataset_name, word_to_count, path_to_count, max_contexts):
    ...
    with open(output_path, 'w') as outfile:
        with open(file_path, 'r') as file:
            for line in file:
                # Added by Spencer Yoder to include file names
                name, line = common.common.separate_name(line)
                # End changes
                ...
                outfile.write('#code_id#' + name + '#code_id#' + target_name + ' ' + " ".join(contexts) + csv_padding + '\n')
                total += 1

...
class common:
    # Added by Spencer Yoder to deal with program hashes being written
    @staticmethod
    def separate_name(line):
        if line.startswith('#code_id#'):
            parts = line.split('#code_id#')
            if len(parts) == 3:
    ...
    def _load_vocab_from_histogram(path, min_count=0, start_from=0, return_counts=False):
        with open(path, 'r') as file:
            ...
            for line in file:
                # Spencer
                _, line = common.separate_name(line)
    ...
    def parse_prediction_results(raw_prediction_results, unhash_dict, special_words, topk: int = 5) -> List['MethodPredictionResults']:
        ...
        for single_method_prediction in raw_prediction_results:
            ...
            # Spencer Yoder
            sorted_attentions = sorted(single_method_prediction.attention_per_context,
                                       key=lambda x : single_method_prediction.attention_per_context[x][0], reverse=True)
            topk_attention_per_context = [
                (key, single_method_prediction.attention_per_context[key][0], single_method_prediction.attention_per_context[key][1])
                for key in sorted_attentions
            ][:topk]
            for context, attention, vector in topk_attention_per_context:
                ...
                if hashed_path in unhash_dict:
                    ...
                    current_method_prediction_results.append_attention_vector(attention.item(), vector)
...
class MethodPredictionResults:
    ...
    def append_attention_vector(self, attention_score, vector):
        self.attention_vectors.append({'score': attention_score, 'vector': vector})

...
TRAIN_DIR='raw_code/Train'
VAL_DIR='raw_code/Validate'
TEST_DIR='raw_code/Test'
...
# Added by Spencer Yoder
mkdir data/${DATASET_NAME}/with_ids
cp data/${DATASET_NAME}/*.c2v data/${DATASET_NAME}/with_ids
for file in data/${DATASET_NAME}/*.c2v; do
  if [[ "$file" != *"dict"* ]]; then
    echo "$file"
    ${PYTHON} remove_ids.py "$file" > "$file.tmp"
    mv "$file.tmp" "$file"
  fi
done

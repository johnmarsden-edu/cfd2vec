    ...
    def predict(self):
        ...
        while True:
            ...
            for raw_prediction, method_prediction in zip(raw_prediction_results, method_prediction_results):
                ...
                print('Attention:')
                for attention_obj, vector_obj in zip(method_prediction.attention_paths, method_prediction.attention_vectors):
                    print('%f\tcontext: %s,%s,%s' % (
                    attention_obj['score'], attention_obj['token1'], attention_obj['path'], attention_obj['token2']))
                    print('vector: ', end='')
                    print(vector_obj['vector'][:4], end='')
                    print('... %d' % len(vector_obj['vector']))
                    print()

...
class Config:
    ...
    def arguments_parser(cls) -> ArgumentParser:
        ...
        #Added by Spencer Yoder, custom prediction task
        parser.add_argument('-cp', '--custom_predictions', required=False,
                            help='The name of the csv file to use containing the training labels')
        return parser
    ...
    def load_from_args(self):
        ...
        #Added by Spencer Yoder, custom prediction task
        self.CUSTOM_PREDICTIONS = args.custom_predictions
    ...
    def get_vocabularies_path_from_model_path(cls, model_file_path: str) -> str:
        ...
        # Changed by Spencer Yoder to work with Windows
        # return '/'.join(model_file_path.split('/')[:-1] + [vocabularies_save_file_name])
        return os.path.join(os.path.split(model_file_path)[0], vocabularies_save_file_name)
    ...
    def model_load_dir(self):
        ...
        return os.path.split(self.MODEL_LOAD_PATH)[0]

    ...
    G: NodeIndexable,
{
    ...
    where
        W: Write,
    {
        let mut config = EmitterConfig::new().perform_indent(self.pretty_print);
        ...
        W: Write,
    {
        ...
        // emit graph to buffer with nodes/edges and possibly weights
        let buff = Cursor::new(Vec::new());
        let tabber = if self.pretty_print { "\n  " } else { "" };
        let mut graph_writer = EventWriter::new_with_config(
            buff,
            EmitterConfig::new()
                .line_separator(tabber)
                .write_document_declaration(false)
                .perform_indent(self.pretty_print),
        );
        self.emit_graph(&mut graph_writer, &mut attributes)?;

        // Emit <key> tags for all the attributes
        self.emit_keys(writer, &attributes)?;

        // Write graph to actual writer now
        if self.pretty_print {
            writer.write(XmlEvent::characters("\n  "))?;
        }
        writer.write(XmlEvent::characters(
            &String::from_utf8(graph_writer.into_inner().into_inner()).unwrap(),
        ))?;
        if self.pretty_print {
            writer.write(XmlEvent::characters("\n"))?;
        }
